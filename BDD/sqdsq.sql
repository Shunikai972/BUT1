...
CREATE TABLE Employe (
    ...
    prenomE VARCHAR2(30),
    ville VARCHAR2(30),
    hebdo NUMBER CHECK (hebdo <= 25),
    numS NUMBER,
    FOREIGN KEY (numS) REFERENCES Service(numS)
);
...
UPDATE Employe SET ville = 'Nantes' WHERE numE = 37;

-- Diminuer de 2h les employés du service 2
UPDATE Employe SET hebdo = hebdo - 2 WHERE numS = 2;
...
UPDATE Travail SET duree = 4 WHERE numP = 103;
...
-- Associer employés aux adresses
...
UPDATE Employe E
SET trav = (SELECT NVL(SUM(T.duree), 0) FROM Travail T WHERE T.numE = E.numE);
