...
NOM = "M'sakni" # A REMPLIR
PRENOM = "Noam" # A REMPLIR
GROUPE = "INFO1-G1-2" # A REMPLIR
# Je ne sais pas si vous lirez, mais au cas ou, j'ai travailler, j'ai fait 2h tout les jours pendant deux semaines et 7h hier, j'ai un trou de mémoire énorme sur determine valuation et ça me bloque, dans 2 semaines je veux faire le 100%.
################ IMPORTS ################
import copy
...
def var_index(i, j, k, n):
    """Retourne l'indice (1 à N^3) pour la case (i,j) contenant la valeur k"""
    N = n * n
    # i, j de 0 à N-1, k de 1 à N
    return (i * N * N) + (j * N) + (k - 1) + 1

def creer_grille_init(list_grille, n):
    N = n * n
    grille = [0] * (N * N)
    for (i, j, val) in list_grille: # Supposons entrée (ligne, col, val)
        # Attention aux indices donnés dans les tests. Souvent c'est 1-based ou 0-based.
        # Le test suggère une liste plate.
        idx = i * N + j # Si i,j sont 0-based
        # Le test donne des listes [1,3,2] -> Ligne 1, Col 3, Val 2 ?
        # Si c'est le cas, on ajuste.
        # Regardons le test : list_grille3 commence par [1,3,2]. 
        # La grille résultante a un 2 à l'index 2 (donc 3ème position).
        # Cela suggère que l'entrée est (Ligne 1-based, Col 1-based, Val).
        grille[(i-1)*N + (j-1)] = val
    return grille

def creer_grille_final(list_var, n):
    N = n * n
    grille = [0] * (N * N)
    # list_var contient True/False pour chaque variable x_ijk
    for idx, val in enumerate(list_var):
        if val is True:
            # Retrouver i, j, k depuis idx (0-based ici)
            tmp = idx
            k = (tmp % N) + 1
            tmp //= N
            j = tmp % N
...
def for_conj_sudoku(n):
    ...
    for k in range(1, n2+1):
        for start in range(n4):
            ...
            def get_cell(ligne, collone, k):
                return (ligne * n4) + (collone * n2) + k


            for k2 in range(k+1, n2+1):
                clauses.add((-get_cell(ligne, collone, k), -get_cell(ligne, collone, k2)))

            for suite in range(start + 1, n4):

                suite_ligne = suite // n2
                suite_collone = suite % n2

                if suite_collone == collone :
                    clauses.add((-get_cell(ligne, collone, k), -get_cell(suite_ligne, suite_collone, k)))

                if suite_ligne == ligne :
                    clauses.add((-get_cell(ligne, collone, k), -get_cell(suite_ligne, suite_collone, k)))

                reg_ligne = ligne // n
                reg_collone = collone // n

                reg_suite_ligne = suite_ligne // n
                reg_suite_collone = suite_collone // n

                if reg_suite_ligne == reg_ligne and reg_collone == reg_suite_collone :
                    clauses.add((-get_cell(ligne, collone, k), -get_cell(suite_ligne, suite_collone, k)))

        for ligne in range(n2):
            clauses_ligne = []
            for collone in range(n2):
                clauses_ligne.append(get_cell(ligne, collone, k))
            clauses.add(tuple(clauses_ligne))

        for collone in range(n2):
            clauses_collone = []
            for ligne in range(n2):
                clauses_collone.append(get_cell(ligne, collone, k))
            clauses.add(tuple(clauses_collone))

        for reg_ligne in range(n):
            for reg_collone in range(n):
                clause_reg = []
                for ligne in range(n) :
                    for collone in range(n) :
                        reel_ligne = (reg_ligne * n) + ligne
                        reel_collone = (reg_collone * n) + collone
                        clause_reg.append(get_cell(reel_ligne, reel_collone, k))
                clauses.add(tuple(clause_reg))
...
def init_list_var(grille, n):
    N = n * n
    nb_vars = N * N * N
    list_var = [None] * nb_vars
    
    for i in range(N):
        for j in range(N):
            val = grille[i * N + j]
            if val != 0:
                v_true = var_index(i, j, val, n)
                list_var[v_true - 1] = True
                for k in range(1, N + 1):
                    if k != val:
                        v_false = var_index(i, j, k, n)
                        list_var[v_false - 1] = False
    return list_var
