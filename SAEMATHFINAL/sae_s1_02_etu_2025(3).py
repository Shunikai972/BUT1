...
def determine_valuations(list_var):
    ...
    for val in list_var : 
        nouvelle_list = []
        if val == None:
            for valuation in resultat:
                nouvelle_list.append(valuation+[True])
                nouvelle_list.append(valuation+[False])
        else:
            for valuation in resultat:
                nouvelle_list.append(valuation+[val])
        resultat = nouvelle_list
    resultat_sans_doublons = []
    for val in resultat:
        if val not in resultat_sans_doublons:
            resultat_sans_doublons.append(val)
    return resultat_sans_doublons
...
'''
    ...
    for possibilite_list_var in list_var : 
        if evaluer_cnf(formule,possibilite_list_var) is True :
            return True , possibilite_list_var
    return False, []
...
'''
    nouvelle_formule = []
    for clause in formule:
        nb_litteral = clause.count(litteral)
        nb_litteral_inv = clause.count(-litteral)
        if nb_litteral == 0:
            if nb_litteral_inv == 0 :
                nouvelle_formule.append(clause)
            else :
                nouvelle_clause = []
                for val in clause :
                    if abs(val) != abs(litteral):
                        nouvelle_clause.append(val)
...
'''
    var_index = 1
    for var_bool in list_var:
        if var_bool is False :
            formule_init = enlever_litt_for(formule_init,-var_index)
        if var_bool is True : 
            formule_init = enlever_litt_for(formule_init,var_index)
...
'''
    ...
    for num , var_bool in list_chgmts:
        num = num + 1
        if var_bool is False :
            num = - num
        formule_init = enlever_litt_for(formule_init,num)
    return formule_init
'''
...
'''
    ...
    for var_bool in list_var :
        if var_bool is None:
            list_chgmts.append((var_index,True))
            list_var[var_index] = True
            break
        var_index = var_index + 1
...
def progress_simpl_for_dpll(formule,list_var,list_chgmts,list_sans_retour):
   ...
   #clause unitaire
    ...
    while index < 5:
        ...
        if compte_inv == 0 and compte !=0:
            formule = enlever_litt_for(formule, num)
            list_chgmts.append((index,True))
            list_var[index]= True
            list_sans_retour.append(num)
            return formule, list_var, list_chgmts , list_sans_retour
        if compte_inv != 0 and compte ==0:
            formule = enlever_litt_for(formule, -num)
...
def retour(list_var,list_chgmts):
    ...
    index_list = len(list_chgmts)-1
    while index_list > -1  :
        index, var_bool = list_chgmts[index_list]
        if var_bool is True :
            list_chgmts[index_list] = (index,False)
            list_var[index] = False
            return list_var, list_chgmts
        list_chgmts.pop(index_list)
        list_var[index] = None
...
'''
    list_var_new = list_var.copy()
    list_chgmts_new = list_chgmts.copy()
    list_sans_retour_new = list_sans_retour.copy()
    
    # On applique le retour sur la dernière variable "modifiable"
    if not list_chgmts_new:
        return formule_init.copy(), list_var_new, list_chgmts_new, list_sans_retour_new

    # On prend la dernière variable True pour la basculer à False
    while list_chgmts_new:
        index, val = list_chgmts_new.pop()
        # si on annule une assignation et que cette variable est marquée
        # sans-retour, on la retire simplement de la liste sans-retour
        # et on continue à remonter (on ne peut pas la basculer)
        if index in list_sans_retour_new:
            list_sans_retour_new.remove(index)
            list_var_new[index] = None
            continue

        # annulation normale
        list_var_new[index] = None  # on "annule" cette variable
        if val is True:
            # On bascule la dernière True trouvée à False
            list_var_new[index] = False
            list_chgmts_new.append((index, False))
            break

    # Reconstruire la formule à partir de formule_init et de list_var_new
    form_new = []
    for clause in formule_init:
        clause_new = []
        for lit in clause:
            var_index = abs(lit) - 1  # les indices commencent à 0
            if list_var_new[var_index] is None:
                clause_new.append(lit)
            elif (lit > 0 and list_var_new[var_index] is True) or (lit < 0 and list_var_new[var_index] is False):
                # clause satisfaite → on peut la supprimer
                clause_new = []
                break
        if clause_new:
            form_new.append(clause_new)

    return form_new, list_var_new, list_chgmts_new, list_sans_retour_new
...
''' 
    ...
    form, list_var, list_chgmts = progress_simpl_for(formule, list_var, list_chgmts)
    
    # Si une clause vide apparaît => backtracking
    if [] in form:
        form_ret, re_list_var, ret_list_chgmts = retour_simpl_for(formule_init, list_var, list_chgmts)
        if ret_list_chgmts == []:
            return False, []
        return resol_parcours_arbre_simpl_for(formule_init, form_ret, re_list_var, ret_list_chgmts)
    ...
    return resol_parcours_arbre_simpl_for(formule_init, form, list_var, list_chgmts)
'''
formule_init= [[1, 2, 4, -5], [-1, 2, 3, -4], [-1, -2, -5], [-3, 4, 5], [-2, 3, 4, 5], [-4, 5]] 
...
'''
    ...
    # Cas d'initialisation du parcours
    if list_chgmts == []:
        if [] in formule:
            return False, []
        if formule == []:
            return True, list_var
        form, list_var_init, list_chgmts_init, list_sans_retour_init = progress_simpl_for_dpll(formule, list_var, [], list_sans_retour)
        return resol_parcours_arbre_simpl_for_dpll(formule_init, form, list_var_init, list_chgmts_init, list_sans_retour_init)

    # Étape générale : appliquer une progression (pure literal / unit / choix)
    form, list_var, list_chgmts, list_sans_retour = progress_simpl_for_dpll(formule, list_var, list_chgmts, list_sans_retour)

    # Si une clause vide apparaît → retour
    if [] in form:
        form_ret, re_list_var, ret_list_chgmts, ret_list_sans = retour_simpl_for_dpll(formule_init, list_var, list_chgmts, list_sans_retour)
        if ret_list_chgmts == []:
            return False, []
        return resol_parcours_arbre_simpl_for_dpll(formule_init, form_ret, re_list_var, ret_list_chgmts, ret_list_sans)
    ...
    return resol_parcours_arbre_simpl_for_dpll(formule_init, form, list_var, list_chgmts, list_sans_retour)
