...
def progress_simpl_for_dpll(formule,list_var,list_chgmts,list_sans_retour):
    list_var_temp = list_var[:]
    list_chgmts_temp = list_chgmts[:]
    
    # Règle du littéral pur et propagation unitaire (simplification déterministe)
    # On commence par la propagation unitaire, et si rien ne se passe, on regarde les littéraux purs
    ...
    while True:
        ...
        if unité is not None:
            var_index = abs(unité) - 1
            valeur = (unité > 0)
            
            if list_var_temp[var_index] is None:
                list_var_temp[var_index] = valeur
                list_chgmts_temp.append((var_index, valeur))
                formule = enlever_litt_for(formule, unité)
                if [] in formule: # Contradiction après simplification
                    return formule, list_var_temp, list_chgmts_temp, list_sans_retour
                list_sans_retour.append(var_index)
                continue # Recommence la boucle while après propagation
        
        # Règle du littéral pur
        litteraux_connus = set()
        litteraux_purs = {} # {var_abs: (litt_val, est_pur)}
        
        # 1. Identifier les littéraux présents dans la formule simplifiée
        for clause in formule:
            for litt in clause:
                var_abs = abs(litt)
                if var_abs not in litteraux_purs:
                    litteraux_purs[var_abs] = (litt, True)
                else:
                    signe_actuel = np.sign(litteraux_purs[var_abs][0])
                    if np.sign(litt) != signe_actuel:
                        litteraux_purs[var_abs] = (litteraux_purs[var_abs][0], False)

        # 2. Appliquer la règle du littéral pur si la variable n'est pas déjà connue
        litt_pur_a_choisir = None
        for var_abs, (litt_val, est_pur) in litteraux_purs.items():
            var_index = var_abs - 1
            if est_pur and list_var_temp[var_index] is None:
                litt_pur_a_choisir = litt_val
                break
        
        if litt_pur_a_choisir is not None:
            var_index = abs(litt_pur_a_choisir) - 1
            valeur = (litt_pur_a_choisir > 0)
            
            list_var_temp[var_index] = valeur
            list_chgmts_temp.append((var_index, valeur))
            formule = enlever_litt_for(formule, litt_pur_a_choisir)
            # Pas besoin de vérifier [] in formule, car un littéral pur ne peut pas causer de contradiction immédiate
            list_sans_retour.append(var_index)
            continue # Recommence la boucle while après simplification
        
        # Si ni propagation unitaire ni littéral pur n'est appliqué, on sort de la boucle de simplification
        break

    # Règle du choix (simplification non déterministe) si la formule n'est pas encore satisfaite
    if formule and not ([] in formule):
        var_a_choisir = None
        for i in range(len(list_var_temp)):
            if list_var_temp[i] is None:
                var_a_choisir = i
                break
        
        if var_a_choisir is not None:
            list_var_temp[var_a_choisir] = True
            list_chgmts_temp.append((var_a_choisir, True))
            formule = enlever_litt_for(formule, var_a_choisir + 1)
...
def retour_simpl_for_dpll(formule_init,list_var,list_chgmts,list_sans_retour):
    ...
    # 1. Annuler les affectations non-déterministes (celles qui ne sont pas dans list_sans_retour)
    while list_chgmts and list_chgmts[-1][0] in list_sans_retour:
        # Si le dernier changement fait partie de list_sans_retour, on remonte jusqu'à lui
        # mais on ne l'annule PAS ici, on passe au retour non-dpll
        break
    
    # Si le dernier changement n'est pas dans list_sans_retour
    if list_chgmts and list_chgmts[-1][0] not in list_sans_retour:
        list_var, list_chgmts = retour(list_var, list_chgmts)
    else:
        # Si tous les changements sont dans list_sans_retour (ou list_chgmts est vide), on ne peut plus revenir en arrière
        list_chgmts = []
        list_var = [None] * len(list_var)
        
    
    # 2. Mettre à jour list_sans_retour (retirer les variables qui ont été remises à None)
    nouvelle_list_sans_retour = []
    variables_changees_indices = {i for i, _ in list_chgmts}
    for index_var in list_sans_retour:
        if index_var in variables_changees_indices:
            nouvelle_list_sans_retour.append(index_var)
        else:
            list_var[index_var] = None

    # 3. Reconstruire la formule
    formule = retablir_for(formule_init, list_chgmts)
    
    return formule, list_var, list_chgmts, nouvelle_list_sans_retour
...
def resol_parcours_arbre(formule_init,list_var,list_chgmts):
    ...
    if not formule:
        # Toutes les clauses ont été satisfaites, la formule est SAT
        # On doit remplir les variables restantes avec True pour obtenir une valuation complète
        list_var_final = list_var[:]
        for i in range(len(list_var_final)):
            if list_var_final[i] is None:
                list_var_final[i] = True
        return True, list_var_final
    
    # Étape de progrès
    formule_nouv, list_var_nouv, list_chgmts_nouv = progress_simpl_for(formule_init, list_var[:], list_chgmts[:])
    
    # Appel récursif avec le nouveau progrès
    sat, val = resol_parcours_arbre(formule_init, list_var_nouv, list_chgmts_nouv)
    
    if sat:
        return True, val
    else:
        # Retour en arrière
        formule_retour, list_var_retour, list_chgmts_retour = retour_simpl_for(formule_init, list_var_nouv, list_chgmts_nouv)
        
        # Si on ne peut plus revenir en arrière (list_chgmts_retour est vide), alors UNSAT
        if not list_chgmts_retour:
            return False, []
        
        # Appel récursif après retour en arrière
        return resol_parcours_arbre(formule_init, list_var_retour, list_chgmts_retour)
...
''' 
    ...
    if list_chgmts==[]:
        if [] in formule:
            return False,[]
        if formule==[]:
            # Cas non atteint à l'initialisation mais conservé par sécurité
            return True,list_var
        form,list_var_init,list_chgmts_init=progress_simpl_for(formule_init,list_var,[])
        return resol_parcours_arbre_simpl_for(formule_init,form,list_var_init,list_chgmts_init)
    
    #Reste du parcours à implémenter :
    if [] in formule:
        # Retour en arrière
        formule_retour, list_var_retour, list_chgmts_retour = retour_simpl_for(formule_init, list_var, list_chgmts)
        
        # Si on ne peut plus revenir en arrière
        if not list_chgmts_retour:
            return False, []
        
        return resol_parcours_arbre_simpl_for(formule_init, formule_retour, list_var_retour, list_chgmts_retour)
    ...
    # Appel récursif avec le nouveau progrès
    sat, val = resol_parcours_arbre_simpl_for(formule_init, formule_nouv, list_var_nouv, list_chgmts_nouv)
...
'''
    ...
    if list_chgmts==[]:
        ...
        if formule==[]:
            # Cas non atteint à l'initialisation mais conservé par sécurité
            return True,list_var
        form,list_var_init,list_chgmts_init,list_sans_retour_init=progress_simpl_for_dpll(formule_init,list_var,[],[])
        return resol_parcours_arbre_simpl_for_dpll(formule_init,form,list_var_init,list_chgmts_init,list_sans_retour_init)

    # Reste du parcours
    if [] in formule:
        # Contradiction, retour en arrière
        formule_retour, list_var_retour, list_chgmts_retour, list_sans_retour_retour = retour_simpl_for_dpll(formule_init, list_var, list_chgmts, list_sans_retour)
        
        # Si on ne peut plus revenir en arrière
        if not list_chgmts_retour:
            return False, []
        
        return resol_parcours_arbre_simpl_for_dpll(formule_init, formule_retour, list_var_retour, list_chgmts_retour, list_sans_retour_retour)
    ...
    # Étape de progrès
    formule_nouv, list_var_nouv, list_chgmts_nouv, list_sans_retour_nouv = progress_simpl_for_dpll(formule_init, list_var[:], list_chgmts[:], list_sans_retour[:])
    
    # Appel récursif avec le nouveau progrès
    sat, val = resol_parcours_arbre_simpl_for_dpll(formule_init, formule_nouv, list_var_nouv, list_chgmts_nouv, list_sans_retour_nouv)
    
    if sat:
        return True, val
    else:
        # Retour en arrière
        formule_retour, list_var_retour, list_chgmts_retour, list_sans_retour_retour = retour_simpl_for_dpll(formule_init, list_var_nouv, list_chgmts_nouv, list_sans_retour_nouv)
...
def creer_grille_init(list_grille,n):
    taille = n * n
    list_grille_complete = [0] * (taille * taille)
    
    for r, c, val in list_grille:
        # Les coordonnées sont 1-basées, on doit les convertir en 0-basées.
        # r: ligne (1 à taille)
        # c: colonne (1 à taille)
        # val: valeur (1 à taille)
        
        # Calcul de l'index dans la liste plate
        index = (r - 1) * taille + (c - 1)
        
        if 0 <= index < len(list_grille_complete):
            list_grille_complete[index] = val
...
def creer_grille_final(list_var,n):
    N = n * n # Taille de la grille (ex: 9 pour n=3)
    list_grille_complete = [0] * (N * N)
    
    # La variable ijk (True) correspond à la variable v = (i-1)*N*N + (j-1)*N + k
    # où i est la ligne (1 à N), j est la colonne (1 à N), et k est la valeur (1 à N).
    
    for i in range(1, N + 1): # Lignes
        for j in range(1, N + 1): # Colonnes
            for k in range(1, N + 1): # Valeurs
                # Index de la variable dans list_var (1-basé)
                var_index_1_based = (i - 1) * N * N + (j - 1) * N + k
                
                # Index de la variable dans list_var (0-basé)
                var_index_0_based = var_index_1_based - 1
                
                # Index de la case dans list_grille_complete
                case_index = (i - 1) * N + (j - 1)
                
                if var_index_0_based < len(list_var) and list_var[var_index_0_based] is True:
                    list_grille_complete[case_index] = k
                    # Une seule valeur True par case (i, j) devrait être trouvée
                    break 
...
list_var_fin=[False, False, False, False, False, False, False, False, True, False, False, True, False, False, False, False, False, False, False, True, False, False, False, False, False, False, False, False, False, False, False, False, True, False, False, False, True, False, False, False, False, False, False, False, False, False, False, False, False, True, False, False, False, False, False, False, False, True, False, False, False, False, False, False, False, False, False, False, False, True, False, False, False, False, False, False, False, False, False, True, False, False, False, False, False, False, False, False, False, False, False, True, False, False, False, False, False, False, True, False, False, False, False, True, False, False, False, False, False, False, False, False, False, False, True, False, False, False, False, False, False, False, False, False, False, False, True, True, False, False, False, False, False, False, False, False, False, False, False, False, False, False, False, True, False, False, False, False, False, True, False, False, False, False, False, False, False, True, False, False, False, False, False, False, False, False, False, False, False, True, False, False, False, False, False, False, False, False, False, True, False, True, False, False, False, False, False, False, False, False, False, False, False, True, False, False, False, False, False, False, False, False, False, True, False, False, False, True, False, False, False, False, False, False, False, False, False, False, True, False, False, False, False, False, False, False, False, False, False, True, False, False, False, False, False, False, False, False, False, False, False, False, True, False, False, False, False, True, False, False, False, False, False, False, False, True, False, False, False, False, False, False, False, False, False, False, False, False, True, False, False, False, False, False, False, False, False, False, False, True, False, True, False, False, False, False, False, False, False, False, False, False, False, True, False, False, False, False, False, False, False, False, False, True, False, False, False, True, False, False, False, False, False, False, False, False, False, False, True, False, False, False, False, False, False, False, False, False, False, True, False, False, False, False, False, False, False, False, False, False, False, False, True, False, False, False, True, False, False, False, False, False, False, True, False, False, False, False, False, False, False, False, False, False, False, False, False, False, True, False, False, False, False, False, False, False, False, False, False, True, False, False, True, False, False, False, False, ]