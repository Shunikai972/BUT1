...
NOM = "BERTRAND" # A REMPLIR
PRENOM = "Noah" # A REMPLIR
GROUPE = "G1-2" # A REMPLIR
...
def evaluer_cnf(formule,list_var):
    for clause in formule :
        if evaluer_clause(clause, list_var) is False :
            return False
        if evaluer_clause(clause, list_var) is None :
            return None
    return True
...
def resol_sat_force_brute(formule,list_var):
    for val in determine_valuations(list_var) :
        if evaluer_cnf(formule,val) is True :
            return True, val
    return False, []
    ...
    for clause in formule :
        ...
        for e in clause :
            if e == litteral :
                sup_clause = True
            if e != -litteral :
                clause_simpl.append(e)
        if not sup_clause :
            nouvelle_formule.append(clause_simpl)
