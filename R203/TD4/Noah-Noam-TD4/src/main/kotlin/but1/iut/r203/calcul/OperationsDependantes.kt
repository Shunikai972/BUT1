 ...
 * @author mottu-jm
 */
class OperationsDependantes(val intProvider: IntProvider) {
    ...
    fun diviserNaturelConsole(): Float {
        val dividende = intProvider.getParam()
        val diviseur = intProvider.getParam()
        if (diviseur == 0) {
    ...
    fun factorielleConsole(): Int {
        var paramConsole = intProvider.getParam()
        var retourFact = 1
