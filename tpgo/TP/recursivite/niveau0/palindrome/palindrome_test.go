package palindrome

import (
	"testing"
)

func TestVide(t *testing.T) {
	if !estPalindrome("") {
		t.Fail()
	}
}

func TestPalindrome1Etudiant(t *testing.T) {
	if !estPalindrome("a") {
		t.Fail()
	}
}

func TestPalindrome2Etudiant(t *testing.T) {
	if !estPalindrome("lol") {
		t.Fail()
	}
}

func TestPalindrome3Etudiant(t *testing.T) {
	if !estPalindrome("kayak") {
		t.Fail()
	}
}

func TestPalindrome4Etudiant(t *testing.T) {
	if !estPalindrome("rionsnoir") {
		t.Fail()
	}
}

func TestPalindrome5Etudiant(t *testing.T) {
	if !estPalindrome("engagelejeuquejelegagne") {
		t.Fail()
	}
}

func TestPasPalindrome1(t *testing.T) {
	if estPalindrome("ab") {
		t.Fail()
	}
}

func TestPasPalindrome2(t *testing.T) {
	if estPalindrome("bonjour") {
		t.Fail()
	}
}

func TestPasPalindrome3(t *testing.T) {
	if estPalindrome("kayaks") {
		t.Fail()
	}
}
